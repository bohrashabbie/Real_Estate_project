 ...
 * `items` map to resolve it against — so a sentinel like "__none__" leaks into
 * the UI instead of its label.
 *
 * Rather than requiring every call site to duplicate its options as an `items`
 * prop, this walks the JSX children for <SelectItem> elements and builds that
 * map itself. It runs at render, not on mount, so labels resolve correctly
 * even while the popup is closed and unmounted. An explicit `items` prop still
 * wins if a caller passes one.
 */
...
function Select<Value>({
  ...
  return (
    <SelectPrimitive.Root
      items={derivedItems as SelectPrimitive.Root.Props<Value>["items"]}
      {...props}
    >
      {children}
    </SelectPrimitive.Root>
